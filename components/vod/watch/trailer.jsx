...
const App = (props) => {
    ...
    const episodes = useMemo(() => {
        let list = new Array();
        let _episodes = _.cloneDeep(props.episodes);

        for(let i = 0; i < _episodes.seasons.length; i ++){
            let season = _episodes.seasons[i];
    ...
    return (
        <>
            ...
            <section className="trailer_episodes_section">
                <h2 className="ssr_only">預告花絮</h2>
                        ...
                        return (
                            <a className={`episode_btn ${getEpisodeBtnFocus(element)}`} href={`/${props.programInfo.content_type}/${element.content_id}`} onClick={(event) => episodeBtnClickHandler(event, element)} target="_self" title={`${element.title}${element.episode_name}`} key={index}>
                                <div className="poster_section">
                                    ...
                                    <LazyLoad placeholder={<Placeholder type="landscape" alt={`${element.title}${element.episode_name}`} />}>
                                        <img className="poster" src={`${cdnstatic}/${element.video_image}`} alt={`${element.title}${element.episode_name}`} />
                                    </LazyLoad>
                                ...
                                <h3 className="text">{element.episode_name}</h3>
                            </a>
                        );
                    })
                }
