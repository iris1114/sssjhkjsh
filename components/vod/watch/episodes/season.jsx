...
const App = (props) => {
    ...
    useEffect(() => {
        glide.current = new Glide(".season_episodes_tab_list", {
            perView: perView,
            bound: true,
            dragThreshold: false,
    ...
    return (
        <>
            ...
            <section className="season_episodes_section">
                <h2 className="ssr_only">劇集列表</h2>
                <h3 className="ssr_only">{group.tabList[listFocus].season_name}</h3>
                ...
                <div className="glide_section">
                    ...
                    <div className="season_episodes_tab_list">
                        <div className="glide__track" data-glide-el="track">
                            <div className="glide__slides">
                                {   
                                    group.tabList.map((element, index) => {
                                        return (
                                            <button className={`glide__slide ${getListBtnFocus(index)}`} onClick={(event) => listBtnClickHandler(event, index)} title={`${element.title}(${element.secondary_mark})`} key={index}>{`${element.season_name} (${element.secondary_mark})`}</button>
                                        );
                                    })
                                }
                            </div>
                ...
                <div className="season_episodes_tab_pane">
                    {   
                        (() => {
                                ...
                                return group.tabPane[listFocus].map((element, index) => {
                                    return (
                                        <a className={`episode_btn ${getEpisodeBtnFocus(element)}`} href={`/${props.episodes.content_type}/${element.content_id}`} onClick={(event) => episodeBtnClickHandler(event, element)} target="_self" title={`${element.title}${element.secondary_mark}`} key={index}>
                                            <h4 className="ssr_only">{element.secondary_mark}</h4>
